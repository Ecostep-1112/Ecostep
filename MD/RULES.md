...
- ✅ **커밋 본문**: 제목 다음 빈 줄 후 상세한 설명 추가
  - 변경 이유, 구체적인 변경 내용, 기술적 세부사항 포함
  - 여러 줄로 작성 가능
...
### Git Push 규칙
- 사용자가 "push 해주세요" 명령 시:
  - 커밋 시 제목과 본문(description) 모두 작성
  - push 후 다음 내용을 사용자에게 제공:
...
장식품을 더블 클릭하면 수정 모드로 진입하도록 구현
수정 모드에서 드래그로 위치 변경 가능
회전 애니메이션 효과 추가

변경된 파일:
- src/components/Decoration.jsx
- src/styles/decoration.css
```

```
fix: 물고기 경계 충돌 버그 수정

물고기가 어항 경계를 벗어나는 버그 수정
충돌 감지 로직 개선 및 경계 체크 추가
```
