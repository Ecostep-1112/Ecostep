  ...
  useEffect(() => {
    ...
    const initialPositions = displayFish.map((fishName, i) => ({
      ...
      y: fishName === '코리도라스' ? 65 :
         ...
         fishName === '네온테트라' ? 40 + (i % 2) * 5 : 35 + (i % 3) * 10,  // 코리도라스는 바닥, 체리바브는 중간, 네온테트라는 중간층, 나머지는 높이 변화
      direction: Math.random() > 0.5 ? 1 : -1,  // 랜덤 방향
      speed: (fishName === '코리도라스' || fishName === '체리바브' || fishName === '네온테트라') ? 0.3 : 0  // 코리도라스, 체리바브, 네온테트라 움직임
    }));
    setFishPositions(initialPositions);

    // 코리도라스, 체리바브, 네온테트라 애니메이션
    const interval = setInterval(() => {
      setFishPositions(prevPositions => {
        return prevPositions.map(fish => {
          if (fish.name === '코리도라스' || fish.name === '체리바브' || fish.name === '네온테트라') {
